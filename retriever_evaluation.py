...
if __name__=='__main__':
    ...
    parser.add_argument('--model', type=str, required=True,
                        choices=['bm25','cohere','e5','google','grit','inst-l','inst-xl',
                                 'openai','qwen','qwen2','sbert','sf','voyage','bge', 'repllama', 'trained_model'])
    
    parser.add_argument('--trainedmodel_name', type=str, default="retriever_MATH_LLMq_and_CoT")
    parser.add_argument('--trainedmodel_hf', type=str, default="RaDeR/retriever_Qwen25_7b_instruct_MATH_LLMq_and_CoT")
    ...
    parser.add_argument('--output_dir', type=str, default='BRIGHT_eval_outputs')
    parser.add_argument('--cache_dir', type=str, default='BRIGHT_cache')
    ...
    if args.reasoning is not None:
        #args.output_dir = os.path.join(args.output_dir,f"{args.task}_{args.trainedmodel_name}_{args.reasoning}_long_{args.long_context}")
        args.output_dir = os.path.join(args.output_dir,f"{args.task}_{args.trainedmodel_name}_{args.reasoning}_instructionTrue_long_{args.long_context}")
    else:
        #args.output_dir = os.path.join(args.output_dir,f"{args.task}_{args.trainedmodel_name}_{args.reasoning}_long_{args.long_context}")
        args.output_dir = os.path.join(args.output_dir,f"{args.task}_{args.trainedmodel_name}_instructionTrue_long_{args.long_context}")
    ...
    if not os.path.isfile(score_file_path):
        ...
        scores = RETRIEVAL_FUNCS[args.model](
            ...
            model_id=args.model, checkpoint= args.checkpoint, trainedmodel_name = args.trainedmodel_name, trainedmodel_hf=args.trainedmodel_hf, **kwargs
        )
    ...
    results = calculate_retrieval_metrics(results=scores, qrels=ground_truth, output_dir=args.output_dir)
