...
from eval_src.toolkit_for_MATH.latex_answer_check import simple_mathcomparison 
...
from prompts.check_equiv_prompt import prompt_equiv
...
class Evaluator:
    ...
    def find_confidence_of_answer(self, completions: List[str], index: int):
        
        if completions is None or len(completions) == 0:
            return None, None, None, None

        #print("OST step which is a terminal node:", completions[index])    
        
        count = 0
        answer2completions = {}
        answer_to_check = None 
        model_answer = self.extract_answer_from_model_completion(completions[index])
        print("Answer to be checked:", model_answer)
        
        for c in completions:
            print(f"Completions {count} :", c)
            if "\\boxed" in c.lower() or "boxed" in c.lower() or "answer is" in c.lower(): 
                
                answer = self.extract_answer_from_model_completion(c)
                print(f"Answer extracted for completions {count}", answer)
                if self.check_answers_equiv(model_answer, answer):
                    count +=1   

        # Calculate confidence 
        total = len(completions)
        
        confidence = (count * 1.0/total)
        ...
        for id, c in enumerate(completions):
            try:
                ...
                for existing_answer in answer2completions.keys():
                    if self.check_answers_equiv(model_answer, existing_answer, gold_answerb=False):
...
class BRIGHTEvaluator(Evaluator):
    def __init__(self) -> None:
        super().__init__()

    def calculate_reward_goldanswer(self, completion, gold_answer, io, question):
        
        # Calculates the reward based on whether the completion matches the gold answer 
        reward = 0 

        # Check if boxed in the completion
        if has_boxed_substring(completion) is False: # If the model solution does not have boxed 
            return 0
        else:
            reward = 0.2  # low reward added since completion is in correct format 
            correct = self.check_answers_equiv(completion, gold_answer)
            if correct is True:  # The gold answer and the completion answer match
                return 1
            else:
                # If correct is False, use an LLM call to determine if the answers are equivalent 
                print("-"*50)
                print("Using LLM Call to check answers!")
                prompt2 = f"\n\nQuestion:{question}\nGold Answer:{gold_answer}\nModel Generated Soution:{completion}\nPrediction: "
                prompt = prompt_equiv + prompt2
              
                print("Model generated solution: ", completion)
                print("Gold Answer: ", gold_answer)
               
                #print("Prompt for checking equivalence: ", prompt)
                
                io_output_list = io.generate(
                        prompt,
                        num_return=1,
                        max_tokens=20,
                        stop_tokens = ["NONE"],
                )
                is_equiv_llm = io_output_list[0].strip()
    ...
    def check_answers_equiv(self, answer_a: str, answer_b: str, gold_answerb = True):
        #print("Model solution", answer_a)
        #print("Gold answer", answer_b)
        ...
        try:
            #print("Model generated solution", answer_a)
            #print("Gold Answer", answer_b)
            #ch = input("Enter the key")
            res = simple_mathcomparison(answer_a, answer_b, gold_answerb)
...
def has_boxed_substring(s):
    pattern = r"boxed\{.*?\}"
    return bool(re.search(pattern, s))

MATHEvaluator = BRIGHTEvaluator 
NUMINA_MATHEvaluator = BRIGHTEvaluator 
